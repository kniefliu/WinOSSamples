...
// ImageData && ImageDataImpl
int decode(const char* filename, std::vector<SkBitmap>& fFrames)
{
	sk_sp<SkData> data(SkData::MakeFromFileName(filename));
	std::unique_ptr<SkCodec> m_codec = SkCodec::MakeFromData(data);
	if (!m_codec) {
		return false;
	}

	int fTotalFrames = m_codec->getFrameCount();

	std::vector<SkCodec::FrameInfo> fFrameInfos = m_codec->getFrameInfo();

	int fTotalDuration = 0;
	...
	if (fTotalFrames == 1 && fFrameInfos.empty()) {
		// It cannot read frame info if only 1 frame found, then add a fake frame info to the fFrameInfos
		fFrameInfos.push_back(SkCodec::FrameInfo());
	}
	for (frameIndex = 0; frameIndex < fTotalFrames; frameIndex++)
	{
		fTotalDuration += fFrameInfos[frameIndex].fDuration;
		SkBitmap& bm = fFrames[frameIndex];
		if (!bm.getPixels()) {
			// Here needs to use kPremul_SkAlphaType for images with alpha channel, otherwise, such images cannot be painted correctly.
			// For detailed information, see its comments in "SkImageInfo.h".
			const SkImageInfo& imgInfo = m_codec->getInfo();
			auto alphaType = imgInfo.isOpaque() ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
			const SkImageInfo info = SkImageInfo::Make(imgInfo.width(), imgInfo.height(), kN32_SkColorType, alphaType);
			bm.allocPixels(info);
			...
			if (SkCodec::kSuccess != m_codec->getPixels(info, bm.getPixels(),
				bm.rowBytes(), &opts)) {
			}

			bm.setImmutable();//all resource set to immutable that the picture recorder can use the same bitmap for texture
		}
	}
...
bool TestDrawGif(SkCanvas* canvas) { 
	...
	//paint.setMaskFilter(SkBlurMaskFilter::Make(kNormal_SkBlurStyle, 10.0f, SkBlurMaskFilter::kHighQuality_BlurFlag));
	paint.setImageFilter(SkBlurImageFilter::Make(1.0f, 1.0f, nullptr));
	canvas->drawBitmap(image, 10, 10, &paint);
	canvas->drawBitmap(image, 200, 10);
	return false;
...
TCHAR s_fontName[64] = TEXT("Î¢ÈíÑÅºÚ");
TCHAR s_text[64] = TEXT("½ñÈÕ  ÍÆ¼ö");
...
bool TestDrawTextTypeFace(SkCanvas* canvas)
{
	...
	textHeight = fontMetrics.fBottom - fontMetrics.fTop;
...
bool TestDrawText2D(SkCanvas* canvas)
{
	SkPaint paint;
	char utf8FontName[64] = { 0 };
	WideCharToMultiByte(CP_UTF8, 0, s_fontName, lstrlen(s_fontName), utf8FontName, 64, NULL, 0);
	...
	int textLen = strlen(text);
	SkRect textBound;
	textWidth = paint.measureText(s_text, textLen * 2, &textBound);
	SkPaint::FontMetrics fontMetrics;
	paint.getFontMetrics(&fontMetrics);
	textHeight = fontMetrics.fBottom - fontMetrics.fTop;

	SkRect rect = SkRect::MakeXYWH(0, 0, 400, 600);
	SkPaint paintR;
	paintR.setStyle(SkPaint::kFill_Style);
	SkColor fillColor = SkColorSetARGB(128, 0xff, 0xff, 0);
	paintR.setColor(fillColor);
	canvas->drawRect(rect, paintR);

	//SkBitmap image;
	//SkPaint paintBMP;
	//std::vector<SkBitmap> bitmaps;
	//decode("menu_bk.png", bitmaps);
	//canvas->drawBitmap(bitmaps[0], 0, 60, &paintBMP);
	
	//


	
	SkColor textColor = SK_ColorRED;//SkColorSetARGB(0xff, 0x55, 0x55, 0x55);
	...
	canvas->drawText(s_text, textLen * 2, textX, textY + textSize - fontMetrics.fLeading / 2, paint);
